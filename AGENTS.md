...
- **Write tests** (mutating DB) use `it.concurrent` and wrap the body in `isolateInTransaction(async () => {...}, txHost)` from `test/shared/utils/isolateInTransaction.ts` — the body runs inside a DB transaction that rolls back, preventing test pollution. Fetch the context with `await testApp.context()` which returns `{ app, txHost, now }`
- **Read-only tests** (asserting against committed seed rows) use `it.concurrent` and fetch the context with `await testApp.context()`
...
- `freezeDate` fakes `Date` for the whole file (one frozen clock). Tests derive expected timestamps from the `now` in the testApp context; fixtures carry UTC instants
